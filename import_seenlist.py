...
def get_movies_list():
    ...
    with open(csv_filename, 'r', encoding = 'ISO-8859-1') as f:
        ...
        for row in csv_reader:
            if line_count > 0:
                line_count += 1
                movie_with_year = row[3] + ' ' + row[8]
                movies_list.append(movie_with_year)
            else:
                line_count += 1
    return movies_list

def add_to_seenlist(obj_id, obj_type):
    justwatch_id = ''
    authorization = ''
    url = ('https://apis.justwatch.com/personalization/title_list/seenlist/object_type/{0}/object_id/{1}?justwatch_id={2}').format(obj_type, obj_id, justwatch_id)
...
if __name__ == '__main__':
    ...
    for movie in movies_list:
        ...
        try:
            ...
            add_to_seenlist(object_id, object_type)
        except Exception as e:
            print("The title " + movie + " could not be found in the justwatch website. So skipping it...")
            continue
        print('added ' + movie + ' to your seenlist')
        sleep(3)
    print('finished importing movies and shows to your justwatch seenlist')
